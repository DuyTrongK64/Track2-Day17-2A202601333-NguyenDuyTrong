...
def main() -> int:
    ...
    if n_src == 0:
        print("  chưa có file nguồn — hãy chạy `make seed-extra` trước.")
        return 1

    old_rows = con.execute(f"select count(*) from read_parquet('{SRC}/*.parquet')").fetchone()[0]
    ...
    new_rows = con.execute(f"select count(*) from read_parquet('{DST}/*/*.parquet')").fetchone()[0]
    n_dst = len(list(DST.glob("*/*.parquet")))
    print(f"  đích  : {DST}  ({n_dst:,} file, {new_rows:,} hàng)")
    assert old_rows == new_rows, f"Lệch số hàng: {old_rows} != {new_rows}"
    print("  compact thành công!")
    return 0
