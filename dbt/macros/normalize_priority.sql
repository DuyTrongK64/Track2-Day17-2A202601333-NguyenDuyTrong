...
{% macro normalize_priority(col) %}
    case
        when lower(trim(cast({{ col }} as varchar))) in ('1', 'urgent') then 1
        when lower(trim(cast({{ col }} as varchar))) in ('2', 'high')   then 2
        when lower(trim(cast({{ col }} as varchar))) in ('3', 'medium') then 3
        when lower(trim(cast({{ col }} as varchar))) in ('4', 'low')    then 4
        else null
    end
{% endmacro %}
...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or trim(cast({{ col }} as varchar)) = '' then 'giá trị rỗng hoặc NULL'
        when try_cast({{ col }} as integer) is not null then 'giá trị số ngoài khoảng 1..4'
        else 'nhãn chuỗi không hợp lệ'
    end
{% endmacro %}
